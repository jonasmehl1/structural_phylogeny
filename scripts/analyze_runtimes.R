...
# opt <- NULL
# opt$alnstats <- "test/stats/aln.tsv"
# opt$time <- "test/stats/a.tsv"
...
df <- read_delim(opt$time, col_names = cnames) %>% 
  mutate(max_rss_gb = measurements::conv_unit(max_rss, from = "MB", to = "GB"))

aln_stats <- read_delim(opt$alnstats) %>%
...
ggsave(opt$outfile, final_plot, width = 9, height = 6, dpi = 300)
