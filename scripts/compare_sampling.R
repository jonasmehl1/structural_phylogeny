...
taxidmap <- read_delim(opt$taxidmap, col_names = c("target", "Tax_ID"), show_col_types=F)

tax <- read_delim(opt$meta, show_col_types=F)
# read input table
...
table <- read_delim(opt$input, show_col_types=F, 
...
blast_self <- read_delim(opt$self_blast, col_names = blast_columns) %>% 
  filter(query==target) %>% 
  mutate(blast_selfbit=bitscore) %>% 
  select(query, blast_selfbit)

fs_self <- read_delim(opt$self_fs, col_names = fs_columns) %>% 
  filter(query==target) %>% 
...
ggsave(opt$outfile, final_plot, width = 14, height = 10, dpi = 300)
...
ggsave(opt$outfile2, saturation_plot, width = 7, height = 5, dpi = 300)
