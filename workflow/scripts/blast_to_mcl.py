"""
input: concatenate all the raw all-by-all blast results into one file
...
qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qcovs qcovhsp qlen slen staxids
...
# Source:
# https://bitbucket.org/yangya/phylogenomic_dataset_construction/src/master/blast_to_mcl.py
...
def blast_to_mcl(blast_output,hit_frac_cutoff):
	# ,exclude=EXCLUDE,\
	...
	for line in infile:
		...
		# 	continue
		pident = float(spls[2])
		qlen,qstart,qend = float(spls[14]),float(spls[6]),float(spls[7])
		slen,sstart,send = float(spls[15]),float(spls[8]),float(spls[9])
