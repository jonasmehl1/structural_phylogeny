...
def Tajima_dist(kn_ratio, bfactor=1, iter = 100, delta=10**-5):
	#approx of b*log(1-p/b) from https://doi.org/10.1093/oxfordjournals.molbev.a040031
	#set all entries to smaller than bfactor
	kn_ratio[kn_ratio > bfactor] = bfactor - delta
...
def runFoldseekdb(folder , outfolder , foldseekpath = 'foldseek'):
...
def runFoldseek_allvall(structfolder, outfolder, foldseekpath = 'foldseek', maxseqs = 3000):
	'''
...
def runFoldseek_allvall_EZsearch(infolder, outpath, tmppath, foldseekpath = 'foldseek'):
	'''
	...
	args = foldseekpath + ' easy-search ' + infolder + ' ' + infolder +' '+ outpath + ' ' + tmppath + " --format-output 'query,target,fident,alnlen,mismatch,gapopen,qstart,qend,tstart,tend,evalue,bits,lddt,lddtfull,alntmscore' --exhaustive-search"
	p = runargs(args)
	return outpath
...
def runFastme(fastmepath, clusterfile):
	...
	args =  fastmepath +  ' -i ' + clusterfile + ' -o ' + clusterfile+'.tmp -n '
	p = runargs(args)
	return clusterfile+'.tmp'
