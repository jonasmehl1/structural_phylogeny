...
def make_lineages(uniprot_df):
	return dict(zip(uniprot_df['mnemo'] 
		, uniprot_df['tax'].map( lambda x :  set( x.split(',') ) ) ) )

def label_leaves(tree, taxidmap, uniprot_df):
	"""
	...
	leaf_lineages = make_lineages(uniprot_df)
	for n in tree.treenode.iter_leaves():
		taxid = taxidmap[taxidmap['query']==n.name].iloc[0]['species']
		n.name = uniprot_df[uniprot_df['query']==taxid].iloc[0]['mnemo']
