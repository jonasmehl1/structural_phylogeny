...
fig1c <- tibble(seeds) %>% 
  mutate(`Has > 4 common` = seeds %in% paste0("AF-",aln_seeds,"-F1")) %>% 
  ggplot(aes(fill=`Has > 4 common`, y=1)) + 
  geom_bar() +
  coord_cartesian(expand = 0) +
  scale_fill_manual(values = c("grey80", "black")) +
  labs(x="Has > 4 Common hits") +
...
sps <- read_delim("data/input/Hsapopi_set_sp.txt")

sptree_plot <- fortify(sptree) %>% 
  left_join(sps, by=c("label"="mnemo")) %>% 
  mutate(species=gsub("[^A-Z].+? ", ". ", species)) %>% 
  ggtree() +
  geom_tiplab(aes(label=species), nudge_x=.2, fontface="italic") + 
  geom_nodelab(color="firebrick", 
               nudge_x = -.15, hjust = 1, 
               nudge_y = .3, size=2.5)+
  scale_x_continuous(expand = expansion(mult = c(0.2, .7)))

ggsave("draft/figures/Fig1/tree.pdf", sptree_plot, width = 4, height = 4)

fig1 <- (fig1b | (fig1c/fig1d + plot_layout(heights = c(10,1)))) +
#   plot_layout(widths = c(1,2)) + 
#   plot_annotation(tag_levels = 'A') & 
#   theme(plot.tag = element_text(size = 10, face = "bold", family = "Helvetica"))
ggsave("draft/figures/Fig1/fig1a.pdf", fig1a, width = 3, height = 4)
ggsave("draft/figures/Fig1/fig1b.pdf", fig1b, width = 4, height = 2)
ggsave("draft/figures/Fig1/fig1c.pdf", fig1c, width = 4, height = 1)
...
var_r2t <- scores %>% 
  ...
  ggplot(aes(fill=model, variance_r2t)) + 
