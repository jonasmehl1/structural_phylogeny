...
CATH_plot <- df_multi %>% 
    ...
    ggplot(aes(Jaccard, fill=singleton)) +
...
ggsave("draft/figures/fig2.pdf", fig2, width = 9, height = 6)
```
...
plot_bs <- df_bs %>% 
  ...
  ggplot(aes(model, support, fill=target)) +
...
fig3 <- rf_plot | ((plot_bs | plot_DL) / var_r2t)
...
fig_s1 <- (plot_dist + sptree_plot + pp1_plot) + plot_layout(widths = c(1.4, 1, 1.6))
